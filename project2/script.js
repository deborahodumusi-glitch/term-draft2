...
const data = rawData.map(d => ({
    ...
    quality: d.quality // Keeping raw quality (1-7) as per user request/image
}));

// 3. VISUALIZATION SETUP (D3.js)
// Increased right margin to make room for the vertical legend and title
...
// Y Axis: Sleep Quality (0-5)
const y = d3.scaleLinear()
    .domain([0, 5.5]) // Scale 0-5 with padding
    .range([height, 0]);
...
// 5. THE 3RD FACTOR: COLOR SCALE (Mental Well-being)
...
// Bubble size varies by wellbeing rating (larger = higher wellbeing)
...
svg.append("g")
    ...
    .attr("cy", d => y(d.quality)) // Y is Quality now
    .attr("r", d => sizeScale(d.wellbeing)) // Size by wellbeing
    .style("fill", d => color(d.wellbeing)) // Color is Wellbeing
    .style("opacity", 0.85)
    .style("stroke", "#333")
    .style("stroke-width", 1.5)
    .on("mouseover", function (event, d) {
        d3.select(this).attr("r", sizeScale(d.wellbeing) + 5).style("opacity", 1);
    ...
    .on("mouseout", function (d) {
        d3.select(this).attr("r", d => sizeScale(d.wellbeing)).style("opacity", 0.85);
